...
let resan = document.getElementById('resan')
let valores = []
...
function Finalizar(){
    ...
    if(valores.length == 0){
        ...
        resan.innerHTML = ''
        resan.innerHTML += `<p>Ao todo, foram cadastrados ${tot} valores.</p>`
        resan.innerHTML +=`<p>O maior valor adicionado foi ${maior}.</p>`
        resan.innerHTML +=`<p>O menor valor adicionado foi ${menor}</p>`
        resan.innerHTML +=`<p>A soma de todos os números foi ${soma}</p>`
        resan.innerHTML +=`<p>A média de todos os números é ${media}</p>`
    }
}
